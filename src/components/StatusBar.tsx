...
function fd(s: number): string {
  return [Math.floor(s / 3600), Math.floor((s % 3600) / 60), s % 60]
    .map((v) => String(v).padStart(2, "0"))
    .join(":");
}

function fr(s: number): string {
...
export default function StatusBar({ startTime, totalDur, testName, onReset }: StatusBarProps) {
  ...
  return (
    <footer className="status-bar">
      ...
      <div className="rm-blk">
        <div className="tm-lbl" style={{ justifyContent: "flex-end" }}>
          ⏰ Temps restant
        </div>
        <div className="rm-val">{fr(remaining)}</div>
        <div className="rm-sub">
          Fin prévue le <strong>{endStr}</strong>
        </div>
      </div>

      <button className="act-btn" onClick={onReset}>
        ↻
      </button>
    </footer>
  );
